  ...
  useEffect(() => {
    const interval = setInterval(() => {
        setTournaments(prevTournaments => {
            ...
            const openTournaments = prevTournaments.filter(t => t.status === 'open' && t.players < t.maxPlayers);
            if (openTournaments.length === 0) return prevTournaments;

            const tournamentToUpdate = openTournaments[Math.floor(Math.random() * openTournaments.length)];
    ...
    }, 2500); // New player joins every 2.5 seconds
